...
export default function BOQPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/boq?projectId=" + encodeURIComponent(projectId), { cache: "no-store" })
      .then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error || "Could not load BOQ items."); return d; })
      .then(d => setItems(Array.isArray(d) ? d : []))
      .catch(e => setError(e instanceof Error ? e.message : "Could not load BOQ items."))
  ...
  async function save(e: FormEvent) {
    ...
    if (!form.itemNo.trim() || !form.description.trim()) { setError("Item number and description are required."); setSaving(false); return; }
  ...
  return <main className="p-5 md:p-8"><div className="mx-auto max-w-[1500px]">
    <header className="flex flex-col justify-between gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-end"><div><div className="text-[10px] font-bold uppercase tracking-[.16em] text-blue-600">Estimating Worksheet</div><h1 className="mt-1 text-2xl font-bold">Bill of Quantities</h1><p className="mt-1 text-sm text-slate-500">{project ? "Preparing estimate for " + project.name : "Select a project from the top bar to begin."}</p></div><button disabled={!projectId} onClick={add} className="flex items-center justify-center gap-2 bg-blue-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40"><Plus size={16}/> Add BOQ Item</button></header>
    {error && <div className="mt-5 border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
    <div className="mt-5 flex items-center justify-between border border-slate-200 bg-white p-4"><span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Project Focus</span><span className="text-sm font-bold">{project?.name || "No project selected"}</span></div>
    ...
    {project && <div className="mt-4 flex flex-wrap gap-x-8 gap-y-2 border border-slate-200 bg-white px-5 py-4 text-sm"><span>Budget <b>{money(project.budget)}</b></span><span>BOQ / Budget <b>{project.budget ? (total / project.budget * 100).toFixed(1) : "0.0"}%</b></span><span>Remaining <b className={project.budget-total<0?"text-red-600":"text-emerald-700"}>{money(project.budget-total)}</b></span></div>}
