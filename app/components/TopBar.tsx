    ...
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200 bg-white">
        <div className="mx-auto flex h-16 max-w-[1600px] items-center justify-between px-4 md:px-7">
          <div className="flex min-w-0 items-center gap-7">
            <Link href="/" className="flex shrink-0 items-center gap-2.5">
              <span className="flex h-8 w-8 items-center justify-center rounded bg-slate-900 text-[11px] font-black text-white">CF</span>
              <span className="hidden text-[15px] font-bold tracking-tight text-slate-900 sm:block">ConstructFlow</span>
            </Link>
            <div className="hidden h-6 w-px bg-slate-200 lg:block" />
            <button className="hidden items-center gap-2 rounded border border-slate-200 px-3 py-1.5 text-left hover:bg-slate-50 lg:flex">
              <div><div className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Current project</div><div className="text-xs font-semibold text-slate-700">All Projects</div></div>
              <ChevronDown size={14} className="text-slate-400" />
            </button>
          </div>
          <div className="hidden items-center gap-4 md:flex">
            <div className="text-right"><div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Workspace</div><div className="text-xs font-semibold text-slate-700">Project Control</div></div>
            <div className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-[11px] font-bold text-slate-600">CE</div>
          </div>
        ...
        <nav className="hidden border-t border-slate-100 md:block">
          <div className="mx-auto flex max-w-[1600px] items-center gap-0 overflow-x-auto px-4 md:px-7">
            {nav.map(([label, href, Icon]) => {
              const active = href === "/" ? pathname === "/" : pathname.startsWith(href);
              return <Link key={label} href={href} className={`flex h-11 shrink-0 items-center gap-2 border-b-2 px-3 text-[12px] font-semibold transition ${active ? "border-blue-600 text-blue-700" : "border-transparent text-slate-500 hover:text-slate-900"}`}><Icon size={14} strokeWidth={active ? 2.2 : 1.8}/>{label}</Link>;
            })}
            <Link href="/settings" className={`ml-auto flex h-11 items-center gap-2 border-b-2 px-3 text-[12px] font-semibold ${pathname.startsWith("/settings") ? "border-blue-600 text-blue-700" : "border-transparent text-slate-500"}`}><Settings size={14}/>Settings</Link>
          </div>
        </nav>
