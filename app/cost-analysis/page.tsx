...
type Analysis = {
  ...
  estimate: Record<string, number>; spent: Record<string, number>; estimatedTotal: number; actualTotal: number; variance: number; budgetRemaining: number; projectedProfit: number; boqCount: number; entryCount: number; physicalProgress: number; financialProgress: number; budgetUtilization: number; progressGap: number; progressStatus: string; latestProgressDate?: string | null;
};
...
export default function CostAnalysisPage() {
  ...
  async function loadProjects() {
    try {
      ...
      if (requested && list.some((p: Project) => p._id === requested)) setProjectId(requested);
      else if (list[0]) setProjectId((current) => current || list[0]._id);
    } catch {
  ...
  const profitMargin = useMemo(() => data && data.project.contractAmount > 0 ? (data.projectedProfit / data.project.contractAmount) * 100 : 0, [data]);
      ...
      <div className="mx-auto max-w-[1500px]">
        <header className="border-b border-slate-200 pb-5">
          ...
          <div className="mt-1 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div><h1 className="text-2xl font-bold tracking-tight text-slate-900">Cost Analysis</h1><p className="mt-1 max-w-2xl text-sm text-slate-500">Control estimated cost, actual spending, budget exposure, and project progress from one financial register.</p></div>
            <button onClick={loadAnalysis} disabled={!projectId || loading} className="inline-flex items-center justify-center gap-2 border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw size={15} className={loading ? "animate-spin" : ""}/> Refresh analysis</button>
          </div>
        </header>
        ...
        <section className="mt-5 border border-slate-200 bg-white p-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-center"><span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Project under control</span><select value={projectId} onChange={e => setProjectId(e.target.value)} className="w-full max-w-2xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-medium outline-none focus:border-blue-500"><option value="">Select a project</option>{projects.map(p => <option key={p._id} value={p._id}>{p.name}</option>)}</select></div>
        </section>
