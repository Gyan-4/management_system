...
export default function EquipmentPage(){
 ...
 const equipment=new Set(filtered.map(x=>(x.description||"Unassigned").trim())).size;
 const hours=filtered.filter(x=>x.unit.toLowerCase().includes("hour")).reduce((s,x)=>s+Number(x.quantity||0),0);

 function exportCsv(){
   const lines=["Date,Equipment,Provider,Qty,Unit,Rate,Amount",...filtered.map(x=>[x.date,'"'+(x.description||"").replaceAll('"','""')+'"','"'+(x.supplierOrEmployee||"").replaceAll('"','""')+'"',x.quantity,x.unit,x.unitCost,x.amount].join(","))];
   const blob=new Blob([lines.join("\n")],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="equipment-costs.csv";a.click();URL.revokeObjectURL(url);
 }

 return <main className="min-h-screen p-5 md:p-8"><div className="mx-auto max-w-[1500px]">
  <div className="mb-5 border border-slate-200 bg-white p-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><div className="flex items-center gap-2 text-sm font-bold"><Truck size={17}/>Equipment Control</div><p className="mt-1 text-xs text-slate-500">Monitor equipment usage and actual project equipment costs.</p></div><div className="flex flex-wrap gap-2"><select value={projectId} onChange={e=>setProjectId(e.target.value)} className="border border-slate-300 px-3 py-2 text-sm">{projects.map(p=><option key={p._id} value={p._id}>{p.name}</option>)}</select><select value={period} onChange={e=>setPeriod(e.target.value)} className="border border-slate-300 px-3 py-2 text-sm"><option value="all">All records</option><option value="7">Last 7 days</option><option value="30">Last 30 days</option></select><button onClick={exportCsv} disabled={!filtered.length} className="flex items-center gap-2 border border-slate-300 bg-white px-3 py-2 text-sm font-semibold disabled:opacity-40"><Download size={15}/> Export CSV</button></div></div></div>
  <div className="mb-6 grid gap-px border border-slate-200 bg-slate-200 sm:grid-cols-3"><Summary label="Equipment Cost" value={money(total)}/><Summary label="Equipment Entries" value={String(filtered.length)}/><Summary label="Equipment Types" value={String(equipment)}/></div>
  <CostLedger category="Equipment" title="Equipment Register" description="Track equipment usage, rental costs, and equipment-related project expenses." />
 </div></main>
}
